...
- **Format**: ASCII string between 1 and 128 characters (maximum allowable length is 255 characters).
...
```bash
curl -X POST "https://wareply.indexai.co.zw/api/v1/test-idempotency" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
