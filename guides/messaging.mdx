...
<Note>
**Receipt Philosophy**: Do not build applications that depend on handset receipt webhooks from the Developer API. Once a message reaches `status: sent` and receives an `external_message_id`, the API considers the dispatch transaction complete.
</Note>
