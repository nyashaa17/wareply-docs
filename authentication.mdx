...
Developer API access is available on active 7-Day Free Trials and paid commercial plans:

- **Active 7-Day Free Trial**: New workspaces receive full programmatic access to the Developer API with Standard-level capabilities (`api_access` entitlement enabled) for 7 days (168 hours). No credit card is required.
- **Paid Plans**: Workspaces on **Standard**, **Pro**, and **Custom** plans have full programmatic Developer API access.
- **Ongoing Free Plan**: The ongoing Free plan includes 500 Dashboard messages per month for interactive web dashboard use only. It does not include programmatic Developer API access (`api_access: false`).
- **Expired Trials & Inactive Subscriptions**: When a 7-day trial concludes or a paid subscription lapses without renewal, Developer API entitlement is disabled.

If an API key is used against a workspace on the ongoing Free plan, an expired trial, or an inactive subscription, the API rejects the request with HTTP 403 `plan_upgrade_required`:
...
| :--- | :--- | :--- |
| `accounts:read` | List and discover accessible connected WhatsApp business accounts | [`GET /accounts`](/api-reference/accounts/list) |
